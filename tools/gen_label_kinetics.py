...
dataset_path = '/home/shuyu/data/Kinetics/frames_tvl1'
label_path = '/home/shuyu/data/Kinetics/kinetics400'
...
if __name__ == '__main__':
    ...
    for (filename_input, filename_output) in zip(files_input, files_output):
        ...
        for line in lines:
            ...
            k = items[0].strip().replace(' ', '_').replace('"', '').replace('(', '').replace(')', '').replace("'", '')
            folders.append('{}_{}'.format(items[1], dict_categories[k]))
        ...
        for i in range(len(folders)):
            ...
            if not os.path.exists(img_dir):
                ...
                try:
                    ...
                    print('No permission! %s %d %d'%(os.path.join(categories_list[i], curFolder), len(dir_files), curIDX))

...
class TSN(nn.Module):
    def __init__(self, num_class, num_segments, modality,
                 ...
                 is_cpl=False, cpl_feature_dim=128, cpl_scale=2):
    ...
    TSN Configurations:
        ...
        if self.is_cpl:
            in_feat_dim = in_feat_dim_map[base_model]
            self.cpl = CPL(num_class, in_feat_dim, cpl_feature_dim, self.consensus,
                is_shift=self.is_shift, temporal_pool=self.temporal_pool, num_segments=self.num_segments)
            self.cpl_feature_dim = cpl_feature_dim
            self.cpl_scale = cpl_scale
    ...
    def forward(self, input, no_reshape=False):
        ...
        # after reshape base_out [batch_size*num_segments, 2048]
        if self.is_cpl:
            features, centers, dist = self.cpl(base_out)

            dist_logits = F.log_softmax(self.cpl_scale*dist, dim=1)
            # need to return features, centers, dist, dist_logits
        ...
        if self.reshape:
            ...
            if self.is_cpl:
                return output.squeeze(1), features, centers, dist, dist_logits
    ...
    def feature_list(self, input):
        ...
        # out [batch_size*num_segments, 2048]
        out = self.base_model(input.view((-1, sample_len) + input.size()[-2:]))

        # outf [batch_size, num_segments, 2048]
        if self.is_shift and self.temporal_pool:
            outf = out.view((-1, self.num_segments // 2) + out.size()[1:])
        else:
            outf = out.view((-1, self.num_segments) + out.size()[1:])
        # outf [batch_size, 1, num_classes]
        outf = self.consensus(outf)
        # outf [batch_size, num_classes]
        outf = outf.squeeze(1)

        out_list.append(outf)

        if self.dropout > 0:
            # out [batch_size*num_segments, num_classes]
            out = self.new_fc(out)

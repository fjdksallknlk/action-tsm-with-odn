...
# fine tune TSM with kinetics pretrain
pretrain=checkpoint/TSM_kinetics_RGB_${arch}_shift8_blockres_avg_segment8_e50/ckpt.best.pth.tar

CUDA_VISIBLE_DEVICES=${GPU_IDs} python main.py ${dataset} RGB \
     --arch ${arch} --num_segments 8 \
     --gd 20 --lr 0.001 --lr_steps 10 20 --epochs 25 \
     --batch-size 64 -j 8 --dropout 0.8 --consensus_type=avg --eval-freq=1 \
